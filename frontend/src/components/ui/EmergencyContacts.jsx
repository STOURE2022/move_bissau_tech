    ...
    >
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="font-semibold text-gray-800 text-sm">{t('profilePage.emergencyContacts')}</p>
            <p className="text-[10px] text-gray-400">{t('profilePage.emergencySub')}</p>
          </div>
        </div>
        {!adding && (
      ...
      <div className="px-4 pb-3">
        {contacts.length === 0 && !adding && (
          <button onClick={() => setAdding(true)}
            ...
            <p className="text-xs text-gray-400">{t('profilePage.addEmergency')}</p>
          </button>
        )}
        <AnimatePresence>
          {contacts.map(c => (
            <motion.div key={c.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }}
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
              ...
              <div className="space-y-2">
                <input autoFocus type="text" value={name} onChange={e => setName(e.target.value)}
                  placeholder={t('profilePage.contactName')}
                ...
                <input type="tel" value={phone} onChange={e => setPhone(e.target.value)}
                  placeholder={t('profilePage.contactPhone')}
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-brand-500" />
                <div className="flex gap-2">
                  <button onClick={addContact} disabled={!name.trim() || !phone.trim()}
                    ...
                    <Check size={14} /> {t('profilePage.addContact')}
                  </button>
